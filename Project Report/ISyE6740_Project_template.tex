...
To further re-fine the model, we increased the number of dense layers of the CNN model with two additional layers: one with 512 units and one with 256 units. Looking at Table 3, the loss is already lower in the refined model with only one epoch versus the previous two models. The training accuracy is also already higher with only one epoch at 98.93\% in the refined model versus the 73\% of the previous two models. However, the validation accuracy seems to hover around 83\%. This in addition to the accuracy of 99.4\% caused the algorithm to stop at the eigth epoch.
...
\begin{tabular}{lcccccccccc} \hline
\textbf{Epoch}               & \textbf{1}      & \textbf{2}      & \textbf{3}      & \textbf{4}      & \textbf{5}      & \textbf{6}      & \textbf{7}      & \textbf{8} \\
\textbf{Loss}                & 0.0352 & 0.0278 & 0.0315 & 0.0264 & 0.0197 & 0.0303 & 0.0220 & 0.0216\\
\textbf{Train accuracy}      & 0.9893 & 0.9925 & 0.9910 & 0.9928 & 0.9941 & 0.9912 & 0.9934 & 0.9940\\
\textbf{Validation accuracy} & 0.8289 & 0.8300 & 0.8329 & 0.8350 & 0.8289 & 0.8234 & 0.8306 & 0.8314\\ \hline
\end{tabular}%
}
...
In addition to training the model on the labeled dataset, we conducted further testing to evaluate its accuracy and performance on unseen data. To assess its real-world applicability, we collected a set of ten diverse shoe images from the internet, featuring models wearing different types of shoes like heels, sandals, boots, sneakers, and more. We used these images to test the model's ability to accurately identify the shoe classes from these new and unseen examples where the background is not white and the shoes were worn in various poses.

Furthermore, we evaluated the model's performance on the designated test dataset, which was separate from the training data. During this evaluation, the model underwent inference on the test dataset, and the results were promising. The accuracy achieved on the test set was an impressive 82.7\%, indicating the model's capability to generalize well to new and diverse shoe images.
...
Testing Images
\begin{tabular}{lcccccccccc} \hline
\textbf{Test Shoe Class} & \textbf{Predicted Probability} 
Flats
Ankle Boots                & 0.0352 \\
High Boots      & 0.9893\\
Low-Top Sneakers 1
Low-Top Sneakers 2
High-Top Sneakers & 0.8289 \\ \hline
\end{tabular}%
}
\caption{Result of Model Training and Validation Accuracy After Additional Layers with Epochs}
\label{tab:refined_train_loss}
